...
@DisplayName("[통합테스트] 회원 조회 API 테스트")
@KotestIntegrationTest
class UserRetrieveIntegrationTest(
    ...
    fun getMyInfoAPI(accessToken: String) = mockMvcTester
        .get()
    ...
    describe("GET /api/v1/users/me") {
        ...
        context("내 정보 조회 요청이 들어오면") {
            it("200 응답을 반환한다.") {
                // given
                val email = "test@example.com"
                val userId = dbInsertUser(email, createdAt = Instant.parse("2026-01-01T00:00:00Z"))
                val accessToken = jwtTokenProvider.issueAccessToken(JwtUserInfo(userId, email, UserRole.USER.name))

                // when
                val result = getMyInfoAPI(accessToken)

                // then
                result.response.getHeader("X-Service-Name") shouldBe "auth-api"
                result.response.status shouldBe HttpStatus.OK.value()
                val response = result.readBody<SuccessResponse<MyInfoResponse>>(jsonMapper)
                with(response) {
                    code shouldBe 200
                    message shouldBe "OK"
                    ...
                    with(data) {
                        ...
                        role shouldBe UserRole.USER.name
        ...
        context("내 정보 조회요청을 하는데, 회원에 등록되지 않은 유저라면") {
            it("404 응답을 반환한다.") {
                // given
